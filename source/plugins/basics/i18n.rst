...
Blocks makse it extremely easy to define translatable strings. From PHP, just wrap the string in ``Blocks::t()``, and from the templates, just run the string through the ``|t`` filter. Blocks will take the string and check if it has been translated into the user’s preferred language.
